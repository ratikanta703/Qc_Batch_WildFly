...
public class QCProc extends QCStore{
	...
	public static void mopdl_proc() throws ClassNotFoundException, SQLException {
		...
		try {
			...
			resultSet = stmt.executeQuery("Delete  From   Bo_Phone Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Other_Info Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Income Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Address Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Bank_Acnt Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Reference Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Notes Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   ACH_STAGING Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code=590)");
			...
			resultSet = stmt.executeQuery("DELETE FROM    st_ti_comm_log WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  ST_ti_master Where st_code=590)");
		    resultSet = stmt.executeQuery("DELETE FROM    st_ti_co_master WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  ST_ti_master Where st_code=590)");
			resultSet = stmt.executeQuery("DELETE FROM    st_ti_TitleTracking_Log WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  ST_ti_master Where st_code=590)");
			resultSet = stmt.executeQuery("DELETE FROM    st_TI_RCPT A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code=590))");
			resultSet = stmt.executeQuery("DELETE FROM    st_TI_DISB A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code=590))");
			resultSet = stmt.executeQuery("DELETE FROM    st_TI_distribution A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code=590))");
			resultSet = stmt.executeQuery("DELETE FROM    st_TI_disB A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code=590))");		 
			resultSet = stmt.executeQuery("DELETE FROM    st_ti_disb_check_log A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_ti_trans Where iloan_code In (Select iloan_code From st_ti_Master Where st_code in (590)))");		 
			resultSet = stmt.executeQuery("DELETE FROM    st_ti_Trans WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  ST_ti_master Where st_code=590)");
			resultSet = stmt.executeQuery("DELETE FROM    st_ti_schedule WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  ST_ti_master Where st_code=590)");            
			resultSet = stmt.executeQuery("Delete  From   st_ti_master where st_code in (590)");
			...
			resultSet = stmt.executeQuery("Delete  From   BO_VEHICLES Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   TP_BLACKBOOK_VALUES");
			resultSet = stmt.executeQuery("Delete  From   TP_BLACKBOOK_REQUEST_LOG");
			resultSet = stmt.executeQuery("Delete  From   TP_BLACKBOOK_Response_LOG");
			resultSet = stmt.executeQuery("Delete from    ST_TI_REQUEST_STAGING");
			resultSet = stmt.executeQuery("Delete  From   REPAY_DEPOSIT_SCHEDULE Where Bo_Code In (Select Bo_Code   From Bo_Master Where bo_st_code=590)");
			resultSet = stmt.executeQuery("Delete  From   REPAY_DATA_LOG Where Bo_Code In (Select Bo_Code   From Bo_Master  Where bo_st_code=590)");
	...
	public static void mopdl_120_proc() throws ClassNotFoundException, SQLException {
		Connection conn = null;
		...
		try {
			...
			resultSet = stmt.executeQuery("Delete  From   Bo_Phone Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Other_Info Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Income Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Address Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Bank_Acnt Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Reference Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Notes Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code in (120))");
			resultSet = stmt.executeQuery("Delete  From   ACH_STAGING Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code in (120))");
       
			resultSet = stmt.executeQuery("DELETE FROM    st_il_RCPT A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code in (120)))");
			resultSet = stmt.executeQuery("DELETE FROM    st_il_DISB A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code in (120)))");
			resultSet = stmt.executeQuery("DELETE FROM    st_il_distribution A WHERE A.iLOAN_TRAN_CODE IN (SELECT iLOAN_TRAN_CODE  From  st_il_trans Where iloan_code In (Select iloan_code From st_il_Master Where st_code in (120)))");
			resultSet = stmt.executeQuery("DELETE FROM    st_il_Trans WHERE iLOAN_CODE IN (SELECT iLOAN_CODE  From  st_il_master Where st_code in (120))");          
			resultSet = stmt.executeQuery("Delete  From   st_il_master where st_code in (120)");
			resultSet = stmt.executeQuery("Delete  From   Bo_Master Where bo_st_code in (120)");           
		    resultSet = stmt.executeQuery("delete from    bank_ach_staging where st_code in (120)");
			resultSet = stmt.executeQuery("Delete FROM    st_daily_summary where trunc(business_date)>TO_DATE('01-MAY-2018','dd-mon-yy') and st_code in (120)");
			resultSet = stmt.executeQuery("Delete FROM    ca_closing_history where trunc(business_date)>TO_DATE('01-MAY-2018','dd-mon-yy') and st_code in (120)");
	...
	//===================================================================
		   ...
		   resultSet = stmt.executeQuery("Delete FROM    st_daily_summary where trunc(business_date)>TO_DATE('02-JAN-2018','dd-mon-yy') and st_code in (3236)");
		   resultSet = stmt.executeQuery("Delete FROM    ca_closing_history where trunc(business_date)>TO_DATE('02-JAN-2018','dd-mon-yy') and st_code in (3236)");
