...
public class RProcTest extends QCStore{
	...
	public static void proc() throws ClassNotFoundException, SQLException {
		...
		try {
			...
			resultSet = stmt.executeQuery("Delete  From   Bo_Phone Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Other_Info Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Income Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Address Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Bank_Acnt Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Reference Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Notes Where Bo_Code In (Select Bo_Code  From Bo_Master Where bo_st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   ACH_STAGING Where Bo_Code In (Select Bo_Code From Bo_Master Where bo_st_code IN (1258,1259))");
		    resultSet = stmt.executeQuery("delete  from ST_lc_COLLECTIONS_MASTER WHERE LC_CODE IN (SELECT LC_CODE  From  ST_LC_master Where st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("delete  from st_lc_statement WHERE LC_CODE IN (SELECT LC_CODE  From  ST_LC_master Where st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("delete from st_lc_minpay WHERE LC_CODE IN (SELECT LC_CODE  From  ST_LC_master Where st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("DELETE FROM    ST_LC_RCPT A WHERE A.LC_TRAN_CODE IN (SELECT LC_TRAN_CODE  From  ST_LC_trans Where LC_code In (Select LC_CODE From ST_LC_Master Where st_code IN (1258,1259)))");
			resultSet = stmt.executeQuery("DELETE FROM    ST_LC_DISB A WHERE A.LC_TRAN_CODE IN (SELECT LC_TRAN_CODE  From  ST_LC_trans Where LC_CODE In (Select LC_CODE From ST_LC_Master Where st_code  IN (1258,1259)))");
			resultSet = stmt.executeQuery("DELETE FROM    ST_LC_APPORTIONS A WHERE A.LC_TRAN_CODE IN (SELECT LC_TRAN_CODE  From  ST_LC_trans Where LC_CODE In (Select LC_CODE From ST_LC_Master Where st_code  IN (1258,1259)))");
			resultSet = stmt.executeQuery("DELETE FROM    ST_LC_Trans WHERE LC_CODE IN (SELECT LC_CODE  From  ST_LC_master Where st_code IN (1258,1259))");            
			resultSet = stmt.executeQuery("Delete  From   ST_LC_master where st_code in (1258,1259)");
			resultSet = stmt.executeQuery("Delete From   st_lc_statement where LC_CODE IN (SELECT LC_CODE  From  ST_LC_master Where st_code IN (1258,1259))");
			resultSet = stmt.executeQuery("Delete  From   Bo_Master Where BO_ST_CODE  in (1258,1259)");                     
            resultSet = stmt.executeQuery("Delete FROM    st_daily_summary where trunc(business_date)>TO_DATE('01-MAY-2018','dd-mon-yy') and st_code in (1258,1259)");
            resultSet = stmt.executeQuery("Delete FROM    ca_closing_history where trunc(business_date)>TO_DATE('01-MAY-2018','dd-mon-yy') and st_code in (1258,1259)");
